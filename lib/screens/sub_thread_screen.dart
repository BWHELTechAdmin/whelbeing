...
class SubThreadScreen extends StatefulWidget {
  final Reply parentReply;
  final String groupName;

  const SubThreadScreen({
    super.key,
  ...
  @override
  State<SubThreadScreen> createState() => _SubThreadScreenState();
}

class _SubThreadScreenState extends State<SubThreadScreen> {
  ...
  Reply get _parentReply => widget.parentReply;

  List<Reply> get _sortedReplies {
    final replies = List<Reply>.from(_parentReply.replies);
    switch (_sortBy) {
      case 'Top':
        ...
        children: [
          Expanded(
            child: ListView(
              ...
              children: [
                // --- Parent reply as "original post" ---
                _buildParentReplyPost(),
  ...
  Widget _buildParentReplyPost() {
    final vh = SizeConfig.vh;
    final vw = SizeConfig.vw;
    final reply = _parentReply;
    return Container(
      padding: EdgeInsets.all(4.0 * vw),
          ...
          Expanded(
            child: TextField(
              ...
              decoration: InputDecoration(
                hintText: 'Reply to ${_parentReply.author}…',
